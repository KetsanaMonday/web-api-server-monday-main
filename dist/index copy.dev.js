...
var usersRouters = require('./routers/login.routers');
...
app.use("/api", Nationality); // ສ້າງຂໍ້ຜິດພາດກັບ .status. ພວກເຮົາ
...
app.use('/api', function (req, res, next) {
  var key = req.query['Token']; // ຄີບໍ່ຢູ່
  // key isn't present

  if (!key) return next(error(400, 'api key required')); // ຄີບໍ່ຖືກຕ້ອງ

  if (apiKeys.indexOf(key) === -1) return next(error(401, 'invalid api key')); // ທັງຫມົດທີ່ດີ, ເກັບຮັກສາ req.key ສໍາລັບການເຂົ້າເຖິງເສັ້ນທາງ
...
var apiKeys = ['whatday', 'bar', 'baz']; // these two objects will serve as our faux database
...
app.get('/api/user/:name/repos', function (req, res, next) {
  ...
  if (user) res.send(user);else next();
